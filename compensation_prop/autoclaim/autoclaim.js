...
const fs = require('fs');
const merkleTree = JSON.parse(fs.readFileSync('merkle-root/compCompensationMerkleTree.json'));
const claims = merkleTree["claims"];
const distributorAbi = JSON.parse(fs.readFileSync('build/MerkleDistributor.json'))["abi"];
...
async function relayAll(privateKey) {
	...
	const distributorContract = new web3.eth.Contract(distributorAbi, distributorAddress, {from: fromWallet, gasPrice: 1000000000, gas: 130000});
	...
	for(let account in claims) {
		...
		if(!claimed) {
			...
			distributorContract.methods.claim(index, account, amount, proof).send({nonce:initNonce});
			initNonce++;
		}
	}
}

program.version('0.1').requiredOption('-w, --wallet <address>','input the wallet private key to relay transactions from');
